...
d_padding = [d; zeros(M+N-1, 1)];
u_padding = [u; zeros(M+N-1, 1)];

% cross correlation (optimised via FFT)
r_c = xcorr(d_padding, u_padding, M-1, 'biased'); % Compute cross-correlation
R_c = r_c(M:end); % Extract only positive lags

% auto correlation (optimised via FFT)
r_a = xcorr(u_padding, M-1, 'biased'); % Compute auto-correlation
R_a = toeplitz(r_a(M:end)); % Extract only positive lags and construct toeplitz matrix
